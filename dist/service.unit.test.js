...
import { ConfigurationFilePermissionError, ConfigurationNotIngestedError, MissingConfigurationError, MissingConfigurationFileError, UnknownConfigurationFileError, } from "./errors";
    ...
    describe("get", () => {
        describe("when the configuration has been ingested", () => {
            ...
            it("throws a MissingConfigurationError when the requested key is not set", () => {
                const configurationService = ConfigurationService.getInstance();
                expect(() => configurationService.get("KEY")).toThrow(MissingConfigurationError);
            });
        });
    });
    ...
    describe("ingest", () => {
        ...
        describe("when configuration has not been ingested", () => {
            it("throws an UnknownConfigurationFileError if the error lacks a code property", () => {
                ...
                expect(() => ConfigurationService.ingest(path)).toThrow(UnknownConfigurationFileError);
            });
            it("throws a MissingConfigurationFileError if the file does not exist", () => {
                configMock.mockReturnValue({ error: { code: "ENOENT" } });
                expect(() => ConfigurationService.ingest(path)).toThrow(MissingConfigurationFileError);
            });
